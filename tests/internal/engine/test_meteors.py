...
def test_bullet_destroys_meteor_awards_points():
    g = _setup_game_at_level(47)
    g.meteors.append(__import__("internal.resources.enemies.meteor", fromlist=["Meteor"]).Meteor(g.camera_x + 20, 100, getattr(g.image, "meteor_img", None)))
    m = g.meteors[0]
    # Place player bullet overlapping meteor
    b = g.get_pooled_bullet(m.x, m.y, direction=-1, image=getattr(g.image, "bullet_img", None))
...
def test_meteor_collision_invulnerable_does_not_destroy_or_score():
    ...
    g.meteors.append(__import__("internal.resources.enemies.meteor", fromlist=["Meteor"]).Meteor(g.player.x, g.player.y, getattr(g.image, "meteor_img", None)))
    g.update()
